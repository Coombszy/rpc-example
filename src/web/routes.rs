...
use crate::{AppState, web::structs::Application, job::{ApplicationsGeneric, ApplicationGeneric}};
...
pub async fn health( data: Data<AppState>) -> Result<HttpResponse, Error> {
    ...
    if response.is_ok(){
        ...
        Err(actix_web::error::ErrorInternalServerError("Failed to reach gRPC server"))
    }
}

// Create an application
#[post("/application")]
pub async fn create_application( data: Data<AppState>, mut payload: Multipart) -> Result<HttpResponse, Error> {
    ...
    if response.is_ok(){
        ...
        Err(actix_web::error::ErrorInternalServerError("Failed to create new application"))
    }
}
...
pub async fn get_cv( data: Data<AppState>, path: web::Path<String>) -> Result<HttpResponse, Error> {
    ...
    // Create gRPC request
    let request = Request::new(crate::job::GetApplicationRequest{ id: path.into_inner()});
